...
#define CHECK_NVSCI_ERROR(x)                                                                                           \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            throw std::runtime_error(std::string("NvSci Error ") + getNvSciErrorStr(RESULT) +                          \
                                     std::string(" executing NvSci function:\n " #x) +                                 \
                                     std::string("\n at " __FILE__ ":") + std::to_string(__LINE__));                   \
        }                                                                                                              \
    };
...
static void setCpuWaiterAttributes(NvSciSyncAttrList attrList)
{
    ...
    dw::core::Array<NvSciSyncAttrKeyValuePair, 2U> pairArray = {
...
class DWChannelSample
{
  ...
  private:
    ...
    void parseAndCheckArguments()
    {
        ...
        else
        {
            ...
            {
                throw std::runtime_error("Number of consumers cannot exclude to maximum number of consumer this "
                                         "sample.");
            }
            ...
            if (m_type == "SHMEM_LOCAL")
            {
                ...
                {
                    throw std::runtime_error("Number of downStreams (downstream) and Number of Consumers (cons) must "
                                             "equal when use SHMEM, as consumers and producers are in a same process.");
                }
            }
    ...
    {
        if (m_syncMode == SyncMode::BOTH || ((m_syncMode == SyncMode::C2P) && isProducer) ||
            ((m_syncMode == SyncMode::P2C) && !isProducer))
        {
            return [](NvSciSyncAttrList attrList) { setCpuWaiterAttributes(attrList); };
        }
        return {};
    }
    ...
    {
        if (m_syncMode == SyncMode::BOTH || ((m_syncMode == SyncMode::P2C) && isProducer) ||
            ((m_syncMode == SyncMode::C2P) && !isProducer))
        {
            return [](NvSciSyncAttrList attrList) { setCpuSignalerAttributes(attrList); };
        }
        return {};
    }
    ...
    void initChannels()
    {
        ...
        for (size_t consId = 0; consId < m_numConsumers; consId++)
        {
            std::string channelParam =
                "role=consumer,type=" + m_type + ",ip=" + m_ipAddr + ",id=" + m_portId + ",timeout=1000";
            if (m_isMailBox)
            {
            ...
            m_inputPorts[consId] = std::make_unique<dw::framework::PortInput<PacketT>>(ref, getCpuWaiterAttrs(false),
                                                                                       getCpuSignalerAttrs(false));
        ...
        if (m_hasProducer)
        {
            std::string channelParam = "role=producer,type=" + m_type + ",ip=" + m_ipAddr + ",id=" + m_portId +
                                       ",num-clients=" + std::string(getArgument("downstreams")) +
                                       ",producer-fifo=1,fifo-size=4,timeout=1000";
            if (m_type == "NVSCI")
            {
            ...
            PacketTSpec ref = TypeCallbacks<PacketT>::getSpecimen();
            m_outputPort = std::make_unique<dw::framework::PortOutput<PacketT>>(ref, getCpuSignalerAttrs(true),
                                                                                getCpuWaiterAttrs(true));
    ...
    void connectChannels()
    {
        ...
        // Break when: 1. Each producer connect at least one consumer. 2.All consumers are connected. Are both
        // satisfied.
    ...
    void runConsumers(size_t consId)
    {
        ...
        while (1)
        {
            ...
            auto work = [this, inputPacket = inputPtr.get(), waitFence, consId]() {
                ...
                // zs:
                log("helloworld from dwchannel2ros!\n");
                if (c2pEnabled())
                {
    ...
    void printAvgLatency()
    {
        ...
        if (m_hasProducer)
        {
            std::cout << "Producer Send BandWidth: "
                      << (((imageSize * m_maxFrameNumber) / (1024 * 1024)) / (m_prodElapsed / 1000000000.0f)) << "MB/s"
                      << std::endl;
        }
        ...
        for (size_t consId = 0; consId < m_numConsumers; ++consId)
        {
            ...
            if (m_sendLatencyArr[consId].size() != m_maxFrameNumber)
            {
                ...
                std::cout << "Consumer Idx[" << consId
                          << "] Packets Number doesn't match with number of frames, use mailbox mode ? " << std::endl;
            }
            ...
            accumulateLatency += std::accumulate(m_sendLatencyArr[consId].begin() + SKIP_FRAME_NUMBER,
                                                 m_sendLatencyArr[consId].end(), 0);
            std::cout << "Consumer Idx[" << consId
                      << "] Latency: " << (accumulateLatency / (m_maxFrameNumber - SKIP_FRAME_NUMBER)) << "us"
                      << std::endl;
            std::cout << "Consumer Idx[" << consId << "] Recv BandWidth: "
                      << (((imageSize * m_sendLatencyArr[consId].size()) / (1024 * 1024)) /
                          (m_consElapsed[consId] / 1000000000.0f))
                      << "MB/s" << std::endl;
        }
  ...
  public:
    DWChannelSample(const ProgramArguments& args) : m_args(args)
    {
    }
...
int main(int argc, const char** argv)
{
    ...
    ProgramArguments args(
        argc, argv,
        {
            ...
            ProgramArguments::Option_t("prod-stream-names", "",
                                       "colon-separated list of producer nvsciipc endpoints, for NVSCI mode only"),
            ProgramArguments::Option_t("prod-reaches", "",
                                       "colon-separated list of producer reaches (process|chip), for NVSCI mode only"),
            ProgramArguments::Option_t("cons-stream-names", "",
                                       "colon-separated list of consumer nvsciipc endpoints, for NVSCI mode only"),
            ProgramArguments::Option_t("cons-reaches", "",
                                       "colon-separated list of consumer reaches (process|chip), for NVSCI mode only"),
            ProgramArguments::Option_t("sync-mode", "none",
                                       "Type of synchronization (none|p2c|c2p|both), for NVSCI mode only"),
            ProgramArguments::Option_t("dataType", "int", "the type of data to be transfered (int|dwImage|custom)"),
