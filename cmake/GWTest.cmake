...
#
# Adds a googletest framework test to the GW test list
#
...
function(gw_add_tests)
    ...
    if (NOT GW_ADD_TESTS_TARGET)
        message(FATAL_ERROR "No TARGET supplied to gw_add_tests")
    endif()

    if (NOT GW_ADD_TESTS_TEST_TYPE)
        message(FATAL_ERROR "No TEST_TYPE supplied to gw_add_tests")
    endif()

    if (NOT GW_ADD_TESTS_WORKING_DIRECTORY)
        set(GW_ADD_TESTS_WORKING_DIRECTORY "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${CMAKE_CFG_INTDIR}")
    endif()

    get_property(TEST_TYPES GLOBAL PROPERTY TEST_TYPES)

    if (NOT ${GW_ADD_TESTS_TEST_TYPE} IN_LIST TEST_TYPES)
        string_join(" " TEST_TYPES_STR ${TEST_TYPES})
        message(
            FATAL_ERROR
            "Unknown TEST_TYPE ${GW_ADD_TESTS_TEST_TYPE} for target \
            ${GW_ADD_TESTS_TARGET}. Valid options are: ${TEST_TYPES_STR}")
    endif()

    string(TOLOWER ${GW_ADD_TESTS_TEST_TYPE} TEST_TYPE)
    set(HARDWARE_REQUIRED ${GW_ADD_TESTS_HARDWARE_REQUIRED})

    if ((${TEST_TYPE} STREQUAL "unit") AND (${HARDWARE_REQUIRED}))
        message(FATAL_ERROR "GW test (${GW_ADD_TESTS_TARGET}) may not
    ...
    if (NOT DEFINED ENV{GW_LOG_LEVEL})
        list(APPEND GTEST_PROPERTIES "ENVIRONMENT;GW_LOG_LEVEL=I;")
    endif()

    if (NOT DEFINED ENV{GW_ENABLE_LOG_TO_STDOUT})
        list(APPEND GTEST_PROPERTIES "ENVIRONMENT;GW_ENABLE_LOG_TO_STDOUT=1")
    endif()
    ...
    gtest_discover_tests(
        ${GW_ADD_TESTS_TARGET}
        EXTRA_ARGS
            "--gtest_output=xml:TEST-${GW_ADD_TESTS_TARGET}.xml"
        WORKING_DIRECTORY
            ${GW_ADD_TESTS_WORKING_DIRECTORY}
        ${TIMEOUT_VAR_NAME}
            60
        PROPERTIES
            "${GTEST_PROPERTIES}")
    ...
    if ("${CMAKE_CONFIGURATION_TYPES}" STREQUAL "")
        set_property(
            ...
            ${GW_ADD_TESTS_TARGET}${CMAKE_EXECUTABLE_SUFFIX})
