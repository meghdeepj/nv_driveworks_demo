...
std::unique_ptr<RenderNode> RenderNode::create(ParameterProvider& provider) {
  return dw::framework::create<RenderNode>(provider);
}

RenderNode::RenderNode(const RenderNodeParams& params, const dwContextHandle_t ctx)
    : ExceptionSafeProcessNode(std::make_unique<RenderNodeImpl>(params, ctx)) {}
...
DW_REGISTER_NODE(dw::framework::RenderNode)
