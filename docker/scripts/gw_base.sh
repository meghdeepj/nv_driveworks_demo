...
function set_lib_path() {
  ...
  if [ "$RELEASE_DOCKER" == 1 ]; then
    ...
    PY_LIB_PATH=${GW_ROOT_DIR}/py_proto
    PY_TOOLS_PATH=${GW_ROOT_DIR}/modules/tools
...
function determine_bin_prefix() {
  GW_BIN_PREFIX=$GW_ROOT_DIR
  if [ -e "${GW_ROOT_DIR}/bazel-bin" ]; then
    GW_BIN_PREFIX="${GW_ROOT_DIR}/bazel-bin"
  fi
  export GW_BIN_PREFIX
}
...
function start_prof_customized_path() {
  ...
  LOG="${GW_ROOT_DIR}/data/log/${MODULE}.out"
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    PROF_FILE="/tmp/$MODULE.prof"
    rm -rf $PROF_FILE
    BINARY=${GW_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE}
    eval "CPUPROFILE=$PROF_FILE $BINARY \
        --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
        --log_dir=${GW_ROOT_DIR}/data/log $@ </dev/null >${LOG} 2>&1 &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_gdb_customized_path() {
  ...
  eval "gdb --args ${GW_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE} \
      --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
      --log_dir=${GW_ROOT_DIR}/data/log $@"
}
...
if [ $GW_IN_DOCKER = "true" ]; then
  CYBER_SETUP="/ob-detection/cyber/setup.bash"
  if [ -e "${CYBER_SETUP}" ]; then
    source "${CYBER_SETUP}"
  fi
  # create_data_dir
